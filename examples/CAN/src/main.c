...
unsigned int  val_Rx = 0;              /* Globals used for display */
...
{ 
  ...
  LED_Out (val_Tx);                               /* display RX val on LEDs  */
...
// FIXME   __HAL_RCC_PWR_CLK_ENABLE();
  __PWR_CLK_ENABLE();
...
void can1_test (void)
 ...
 { 
  ...
  can1_Init ();                                    /* initialize CAN interface */
  ...
  forever()
  {
    ...
    {                      
      ...
      CAN_TxMsg[0].data[0] = val_Tx;           

     for (int i = 0; i < 8; i++) 
      { 
         CAN_TxMsg[0].data[i] = 0x77;
      }
